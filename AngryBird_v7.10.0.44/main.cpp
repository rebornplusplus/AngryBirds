...
struct varLevel{
	...
	// pig var to control level 4
	int isPig1, isPig2, isKing;
	double pig1PosX, pig2PosX, pig1PosY, pig2PosY, kingPosX, kingPosY;
	double pig1CenX, pig1CenY, pig2CenX, pig2CenY, kingCenX, kingCenY, kingR, pig1R, pig2R;
	...
	varLevel()
	{
		...
		isPig1 = isPig2 = isKing = 1;
		kingPosX = 750, kingPosY = 260-50;
		pig1PosX = 600, pig1PosY = 165-50;
		pig2PosX = 430, pig2PosY = 110-50;
		
		kingCenX = 615, kingCenY = 320-50, kingR = 60;
		pig1CenX = 630, pig2CenY = 195-50, pig2R = 30;
		pig2CenX = 490, pig2CenY = 210-50, pig1R = 45;
...
void level_1()
{
	...
	iShowBMP2(100, 0, "bin/level-4/gulti.bmp", 0);
	iRectangle(20, 20, 80, 80);
...
void level_4() 
{
	if (subLevel == 1)
	{
		...
		if (lev4.flag_1) // any of the bird is flying now.
        {
			// the path function needs to be modified for a better view.
            if (lev4.flyNow == 1) // red is flying
            {
                double time = double(clock() - startingTime)/CLOCKS_PER_SEC/10;
                lev4.redPosX += lev4.velocity * cos(lev4.theta) * time;
                lev4.redPosY += lev4.velocity * sin(lev4.theta) * time - 25 * time * time;
				
				if (distance(lev4.redPosX, lev4.redPosY, lev4.kingCenX, lev4.kingCenY) <= lev4.kingR and lev4.lastHit != 1)
				{
					--lev4.targetsRemaining;
					lev4.lastHit = 1;
					lev4.isKing = false;
				}
				else if (distance(lev4.redPosX, lev4.redPosY, lev4.pig1CenX, lev4.pig1CenY) <= lev4.pig1R and lev4.lastHit != 1)
				{
					--lev4.targetsRemaining;
					lev4.lastHit = 1;
					lev4.isPig1 = false;
				}
				else if (distance(lev4.redPosX, lev4.redPosY, lev4.pig2CenX, lev4.pig2CenY) <= lev4.pig2R and lev4.lastHit != 1)
				{
					--lev4.targetsRemaining;
					lev4.lastHit = 1;
					lev4.isPig2 = false;
                ...
                if (lev4.redPosX < -64 or lev4.redPosX > width or lev4.redPosY < -64 or lev4.redPosY > height)
				{
					--lev4.birdsRemaining;
					lev4.flag_1 = false;
				}
            }
            else if (lev4.flyNow == 2) // chuck is flying
            {
                double time = double(clock() - startingTime)/CLOCKS_PER_SEC/10;
                lev4.chuckPosX += lev4.velocity * cos(lev4.theta) * time;
                lev4.chuckPosY += lev4.velocity * sin(lev4.theta) * time - 25 * time * time;
				
				if (distance(lev4.chuckPosX, lev4.chuckPosY, lev4.kingCenX, lev4.kingCenY) <= lev4.kingR and lev4.lastHit != 2)
				{
					--lev4.targetsRemaining;
					lev4.lastHit = 2;
					lev4.isKing = false;
                ...
                if (lev4.chuckPosX < -64 or lev4.chuckPosX > width or lev4.chuckPosY < -64 or lev4.chuckPosY > height)
                {
					--lev4.birdsRemaining;
					lev4.flag_1 = false;
				}   
            }
            else if (lev4.flyNow == 3) // bomb is flying
            {
                double time = double(clock() - startingTime)/CLOCKS_PER_SEC/10;
                lev4.bombPosX += lev4.velocity * cos(lev4.theta) * time;
                lev4.bombPosY += lev4.velocity * sin(lev4.theta) * time - 25 * time * time;

				if (distance(lev4.bombPosX, lev4.bombPosY, lev4.kingCenX, lev4.kingCenY) <= lev4.kingR and lev4.lastHit != 3)
				{
					--lev4.targetsRemaining;
					lev4.lastHit = 3;
					lev4.isKing = false;
                ...
                if (lev4.bombPosX < -64 or lev4.bombPosX > width or lev4.bombPosY < -64 or lev4.bombPosY > height)
                {
					--lev4.birdsRemaining;
					lev4.flag_1 = false;
				}
        ...
        printf("%lf %lf\n", lev4.chuckPosX, lev4.chuckPosY);
    }
	
	printf("%lf %lf\n", lev4.redPosX, lev4.redPosY);
	
	// now loading the images according to their corresponding co-ordinates
    iShowBMP(0, 0, "bin/level-4/background.bmp");
	iShowBMP2(100, 0, "bin/level-4/gulti.bmp", 0);
	iRectangle(20, 20, 80, 80);
	
    iShowBMP2(lev4.redPosX, lev4.redPosY, "bin/level-4/red.bmp", 0xFFFFFF);
    iShowBMP2(lev4.bombPosX, lev4.bombPosY, "bin/level-4/bomb.bmp", 0);
    iShowBMP2(lev4.chuckPosX, lev4.chuckPosY, "bin/level-4/chuck.bmp", 0xFFFFFF);
	
	
	if (lev4.isPig1) iShowBMP2(lev4.pig1PosX, lev4.pig1PosY, "bin/level-4/pig1.bmp", 0xFFFFFF);
	if (lev4.isPig2) iShowBMP2(lev4.pig2PosX, lev4.pig2PosY, "bin/level-4/pig2.bmp", 0xFFFFFF);
	if (lev4.isKing) iShowBMP2(lev4.kingPosX, lev4.kingPosY, "bin/level-4/king.bmp", 0xFFFFFF);
	...
	// the level has been failed. user will promped to restart. CAUTIONS: needs re-initiating the lev4
	else if (subLevel == 3)
	{
		iText(200, 200, "Level failed. Restart?");
		iFilledRectangle(300, 200, 100, 50);
		...
		if (!strcmp(gameState, "gameplay"))
		{
			if (level == 1)
			{
				if (subLevel == 1)
				{
					...
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev1.redPosX == 100 and lev1.redPosY == 100)
					{
						lev1.flag_1 = true;
						lev1.flyNow = 1;
					...
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev1.chuckPosX == 100 and lev1.chuckPosY == 100)
					{
						lev1.flag_1 = true;
						lev1.flyNow = 2;
						lev1.theta = atan(double(100-my) / (100-mx));
					...
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev1.bombPosX == 100 and lev1.bombPosY == 100)
					{
						lev1.flag_1 = true;
						lev1.flyNow = 3;
						lev1.theta = atan(double(100-my) / (100-mx));
			...
			else if (level == 4)
			{
				if (subLevel == 1)
				{
					if (mx >= 20 and mx <= 84 and my >= 400 and my <= 464 and lev4.redPosX == 20 and lev4.redPosY == 400)
					{
						lev4.redPosX = 100;
						lev4.redPosY = 100;
					}
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev4.redPosX == 100 and lev4.redPosY == 100)
					{
						lev4.flag_1 = true;
						lev4.flyNow = 1;
						lev4.theta = atan(double(100-my) / (100-mx));
						lev4.velocity = sqrt(pow(100-my, 2) + pow(100-mx, 2));
						startingTime = clock();
					}


					if (mx >= 100 and mx <= 164 and my >= 400 and my <= 464 and lev4.chuckPosX == 100 and lev4.chuckPosY == 400)
					{
						lev4.chuckPosX = 100;
						lev4.chuckPosY = 100;
					}
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev4.chuckPosX == 100 and lev4.chuckPosY == 100)
					{
						lev4.flag_1 = true;
						lev4.flyNow = 2;
						lev4.theta = atan(double(100-my) / (100-mx));
					...
					if (mx >= 180 and mx <= 244 and my >= 400 and my <= 464 and lev4.bombPosX == 180 and lev4.bombPosY == 400)
					{
						lev4.bombPosX = 100;
						lev4.bombPosY = 100;
					}
					else if (mx <= 20 and mx <= 100 and my >= 20 and my <= 100 and lev4.bombPosX == 100 and lev4.bombPosY == 100)
					{
						lev4.flag_1 = true;
						lev4.flyNow = 3;
						lev4.theta = atan(double(100-my) / (100-mx));
